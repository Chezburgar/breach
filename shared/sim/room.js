...
export class GameRoom {
  ...
  // -------------------------------------------------------------- players
  /**
   * How many players the mode wants on the board. The lobby fills to this
   * with bots, so it belongs with the mode rather than as a constant in the
   * session — a mode with different team maths would silently get the wrong
   * number otherwise.
   */
  fillTarget() {
    return this.mode.teams
      ? (this.mode.teamCount || 2) * this.mode.teamSize
      : this.mode.maxPlayers;
  }
